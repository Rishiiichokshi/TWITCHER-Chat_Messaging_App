...
import com.example.chatapp.adapters.ChatAdapters;
...
public class ChatActivity extends BaseActivity {
    ...
    private void init() {
        ...
        //making an arraylist for chat messages
        chatMessages = new ArrayList<>();
        chatAdapters = new ChatAdapters(
                chatMessages,
//               this function will convert that string into a image
                getBitmapEncodedString(receiverUser.image),
//        preference manager which takes all the info of user
                preferenceManager.getString(Constants.KEY_USER_ID)
        );
        binding.chatRecyclerView.setAdapter(chatAdapters);
        //get the information from the firebase
        database = FirebaseFirestore.getInstance();

    }

//     so this is the function when user sends the message then
//     the sender id, receiver id, the message and the timestamp(date and time)
//     all of this information will get into a hashmap and when sends the message this
//     info will get to the firebase
    private void sendMessage() {
        ...
        //means that if the user is not a new user(already chatting with) then just update
        // the conversation means that just get the users message
        if(conversionId != null) {
            ...
            //add all of the above info into the hashmap
            addConversation(conversion);
        }
        //to send the message if user is not available
...
//                                showToast(error.getString("error"));
            ...
            public void onFailure(@NonNull Call<String> call, @NonNull Throwable t) {
                //get message will give a message if there is a failure
        ...
        database.collection(Constants.KEY_COLLECTION_USERS).document(
            ...
            if (value != null) {
                ...
                if (receiverUser.image == null) {
                    ...
                    chatAdapters.setRecieverProfileImage(getBitmapEncodedString(receiverUser.image));
...
//            It contains the document affected and a the type of change that occurred (added, modified, or removed).
                if (documentChange.getType() == DocumentChange.Type.ADDED) {
                    ...
                    chatMessage.recieverId = documentChange.getDocument().getString(Constants.KEY_RECEIVED_ID);
    ...
    //this function convert the image string into a bitmap(which is an image)
    ...
    //this will update the conversation whenever the new message is received
